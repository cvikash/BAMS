...
class _ResidualBlock(nn.Module):
    def __init__(
        ...
        self.input_dim = input_size if self.nr_blocks_below == 0 else self.num_filters
        self.output_dim = target_size if self.nr_blocks_below == self.num_layers - 1 else self.num_filters
        self.conv1 = nn.Conv1d(
            self.input_dim,
            self.num_filters,
            self.kernel_size,
            dilation=(self.dilation_base**self.nr_blocks_below),
        )
        self.prelu = nn.ReLU() ##  parametric ReLU activation function
        ...
        if self.input_dim != self.output_dim:
            self.conv3 = nn.Conv1d(self.input_dim, self.output_dim, 1)
...
class TCN(nn.Module):
    ...
    self, input_size: int, kernel_size: int, num_filters: int, num_layers: int,  dilation_base: int, weight_norm: bool,
    target_size: int,
    dropout: float, fin_dim: int, predict_window: int):
    ...
    # x of shape `(batch_size, input_size, num_timesteps)`
        ...
        self.fc1 = nn.Linear(self.n_filters, self.n_filters)
        self.fc2 = nn.Linear(self.n_filters, self.n_filters)
        self.fc3 = nn.Linear(self.n_filters, self.fin_dim*self.predict_window)
    ...
    def forward(self, x):
        ...
        return x.view(x.shape[0], self.fin_dim, self.predict_window)
